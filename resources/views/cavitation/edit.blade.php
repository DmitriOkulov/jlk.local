...
<h1>Изменить Ультразвуковую кавитацию</h1>
    <form method="POST" action="{{ route('cavitation.update', $cavitation) }}">
        ...
        <div class="form-group">
            ...
            <input id="stomach" class="form-control{{ $errors->has('stomach') ? ' is-invalid' : '' }}" name="stomach" type="text" value="{{ old('stomach', $cavitation->stomach) }}">
        ...
        <div class="form-group">
            ...
            <input id="ass" class="form-control{{ $errors->has('ass') ? ' is-invalid' : '' }}" name="ass" type="text" value="{{ old('ass', $cavitation->ass) }}">
            @if ($errors->has('ass'))
        ...
        <div class="form-group">
            ...
            <input id="hips" class="form-control{{ $errors->has('hips') ? ' is-invalid' : '' }}" name="hips" type="text" value="{{ old('hips', $cavitation->hips) }}">
            @if ($errors->has('hips'))
        ...
        <div class="form-group">
            ...
            <textarea name="comment">{{ old('comment', $cavitation->comment) }}</textarea>
        </div>

        <div class="form-group">
            <label for="id_visitor" class="col-form-label">Посетитель (id)</label>
            <input id="id_visitor" class="form-control{{ $errors->has('id_visitor') ? ' is-invalid' : '' }}" name="id_visitor" type="text" value="{{ old('id_visitor', $cavitation->id_visitor) }}">
        </div>
